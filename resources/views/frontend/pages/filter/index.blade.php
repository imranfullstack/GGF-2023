        ...
        $(function(){
            ...
            $('#activmap-wrapper').activmap({
                ...
                @php
                    ...
                    $orgtype = App\Models\Orgtypehave::where('organisation_id',$item->id)->first();

                @endphp

                    {title: '{{$item->org_name}}', orgtype: '{{$orgtype->ogranisationtype->name}}', short_desc: '{{$item->summary}}', url: '{{route('org.index',$item->slug)}}', tags: ['tourism_01'], lat:{{$item->lat}}, lng: {{$item->long}}, img: '{{asset('img/upload/org/logo/'.$item->logo)}}', icon: '{{asset('frontend/images/icons/map.png')}}'},
